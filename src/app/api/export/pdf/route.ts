...
import { generatePdfBuffer } from "@/lib/utils/export-pdf";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const buffer = await generatePdfBuffer(families);

    const filename = familyIdStr && families[0]
      ? `Data_Keluarga_${families[0].familyName.replace(/\s+/g, "_")}_IKKFMS.pdf`
      : "Data_Keluarga_Anggota_IKKFMS.pdf";
